...
uniform float uAnimPhase; // MATRIX-VIZ: integrated animation phase (continuous 3D forward motion)
uniform float overscan; // MATRIX-VIZ: volumetric field overscan (margin for the reactive camera)
...
void main() {
	...
	if (volumetric) {
		...
		quadDepth = fract(startDepth + uAnimPhase * forwardSpeed); // MATRIX-VIZ: continuous (was time*animationSpeed)
		vDepth = quadDepth;
	}
	...
	if (volumetric) {
		pos.xy *= overscan; // MATRIX-VIZ: spread the field past the frame so the camera has margin
