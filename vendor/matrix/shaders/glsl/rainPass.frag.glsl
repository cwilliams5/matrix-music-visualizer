...
#define TITLE_N 8
uniform sampler2D uTitleMask; // MATRIX-VIZ (plan-transitions): atlas of TITLE_N stacked title masks
uniform float uTitleAmount[TITLE_N]; // MATRIX-VIZ: Mask (burn-in) envelope per slot (0 = none)
uniform vec2 uResolution; // MATRIX-VIZ: render-target size, so the title mask samples in SCREEN space
...
void main() {
	...
	// MATRIX-VIZ (plan-transitions): the "Mask" (burn-in) song titles — boost glyph brightness inside the
	// title mask, sampled in SCREEN space (gl_FragCoord / uResolution) so it reads in BOTH 2D and volumetric
	// 3D. N concurrent SLOTS that STACK (a new title claims a new slot); each slot's mask is a band of the
	// atlas. As a slot's amount fades, its boosted glyphs return to normal.
	if (!showDebugView) {
		vec2 titleUV = vec2(gl_FragCoord.x / uResolution.x, 1.0 - gl_FragCoord.y / uResolution.y);
		for (int i = 0; i < TITLE_N; i++) {
			if (uTitleAmount[i] > 0.001) { // uniform branch (per-slot)
				float atlasV = (float(i) + clamp(titleUV.y, 0.0, 1.0)) / float(TITLE_N); // this slot's band
				float titleMask = texture2D(uTitleMask, vec2(titleUV.x, atlasV)).r;
				brightness.x += titleMask * uTitleAmount[i] * 1.8;
			}
		}
	}
