// MATRIX-VIZ: asset base-path support so the renderer can live under vendor/matrix/
// while the app serves from a different web root. Empty base = original behavior.
let assetBase = "";
const setAssetBase = (b) => {
	assetBase = b ?? "";
};
const resolveURL = (url) => (url == null || /^(https?:|data:|blob:|\/)/.test(url) ? url : assetBase + url);
	...
	return {
		...
		loaded: (async () => {
			if (url != null) {
				...
				data.src = resolveURL(url); // MATRIX-VIZ
				await data.decode();
				loaded = true;
				if (mipmap) {
...
const loadText = (url) => {
	...
	return {
		...
		loaded: (async () => {
			if (url != null) {
				text = await (await fetch(resolveURL(url))).text(); // MATRIX-VIZ
				loaded = true;
			}
		})(),
	};
...
export { makePassTexture, makePassFBO, makeDoubleBuffer, loadImage, loadText, makeFullScreenQuad, make1DTexture, makePass, makePipeline, setAssetBase };
